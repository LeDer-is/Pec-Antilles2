...
function Kpi({ label, value, color, delay }: { label: string; value: string; color: string; delay: number }) {
  return (
    <div className="bg-raised border border-white/5 rounded-lg p-2.5 sm:p-3 md:p-4 animate-fade-in-up overflow-hidden min-w-0" style={{ animationDelay: `${delay * 60}ms` }}>
      <div className="text-[8px] sm:text-[9px] md:text-[10px] text-slate-400 uppercase tracking-wider mb-1 truncate">{label}</div>
      <div className={`text-xs sm:text-sm md:text-lg font-bold ${color} truncate`}>{value}</div>
    </div>
  );
}
